...
OUTPUT_DIR = ROOT / "outputs" / "multimodal_lobewise_svm" / "shap_stability"
...
def parse_feature_name(name: str) -> dict[str, str]:
    """Parse a feature name like T1GD_temporal_en_ratio into components."""
...
def load_artifacts() -> tuple[Pipeline, pd.DataFrame, pd.Series]:
    """Load SVM pipeline and merged multimodal dataset.

    Uses ONLY SVM artifacts — no XGBoost models or TreeExplainer.
    """
    ...
    assert list(X.columns) == model_features, \
        "Feature columns do not match SVM model training features."
    return model, X, y
...
def single_bootstrap_shap(
    ...
    """Run one bootstrap repetition of SVM SHAP computation using KernelExplainer."""
...
def main():
    log("=" * 60)
    log("  SVM SHAP RANKING STABILITY — BOOTSTRAP EVALUATION")
    log("=" * 60)
    ...
    log(f"  Model: SVM pipeline (StandardScaler + SVC RBF) | {n_features} features")
    log(f"  Dataset: {X.shape[0]} patients")

    log(f"\n[2/4] Running {N_BOOTSTRAP} bootstrap repetitions (KernelExplainer) ...")
    log(f"  Background size: {BACKGROUND_SIZE}")
    log(f"  Explained instances: {N_EXPLAIN}")
    log(f"  nsamples: {min(200, 2 * n_features + 2048)}")
    ...
    json_data = {
        "metadata": {
            "model": "SVM RBF (StandardScaler + SVC)",
    ...
    ax.boxplot(
        ...
        vert=False, tick_labels=sorted_names, patch_artist=True,
        boxprops=dict(facecolor="#2166AC", alpha=0.6),
        medianprops=dict(color="black", lw=1.5),
        whis=(2.5, 97.5),
    )
    ax.set_xlabel("Mean |SHAP|")
    ax.set_title("SVM SHAP Importance Distribution Across Bootstrap Repetitions")
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / "shap_rank_distribution.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    log(f"  Saved shap_rank_distribution.png")
    ...
    ax.set_title("SVM SHAP Rank of Top-10 Features Across Bootstrap Repetitions")
    ...
    plt.close(fig)
    log(f"  Saved shap_top10_stability.png")

    top10 = stability_df.head(10)

    queries = {
        "T1GD_temporal_en_ratio": stability_df[
            stability_df["feature_name"] == "T1GD_temporal_en_ratio"
        ],
        "T1GD_frontal_en_ratio": stability_df[
            stability_df["feature_name"] == "T1GD_frontal_en_ratio"
        ],
        "T1_tumor_burden_index": stability_df[
            stability_df["feature_name"] == "T1_tumor_burden_index"
        ],
        "T1_temporal_nc_ratio": stability_df[
            stability_df["feature_name"] == "T1_temporal_nc_ratio"
        ],
    }
    query_results = {}
    for qname, qdf in queries.items():
        if len(qdf) > 0:
            query_results[qname] = {
                "freq_top5_pct": qdf["freq_top5_pct"].values[0],
                "freq_top10_pct": qdf["freq_top10_pct"].values[0],
                "mean_abs_shap": qdf["mean_abs_shap"].values[0],
                "median_rank": qdf["median_rank"].values[0],
            }
        else:
            query_results[qname] = {
                "freq_top5_pct": 0.0,
    ...
    log(f"\n{'=' * 60}")
    log(f"  SVM SHAP stability complete. Outputs in: {OUTPUT_DIR}")
    log(f"{'=' * 60}")
    return 0
