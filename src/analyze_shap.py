...
def main():
    ...
    t1gd_temporal_en = stability_df[
        stability_df["feature_name"] == "T1GD_temporal_en_ratio"
    ]
    t1gd_frontal_en = stability_df[
        stability_df["feature_name"] == "T1GD_frontal_en_ratio"
    ]

    t1gd_temporal_en_top5 = (
        t1gd_temporal_en["freq_top5_pct"].values[0]
        if len(t1gd_temporal_en) > 0 else 0.0
    )
    t1gd_frontal_en_top5 = (
        t1gd_frontal_en["freq_top5_pct"].values[0]
        if len(t1gd_frontal_en) > 0 else 0.0
    )
